...
async function load(){try{const r=await fetch("data/vocabulary.json?v=2", { cache: "no-store" });if(!r.ok)throw Error("Data error");vocabulary=await r.json();loadProgress();renderWeeks();renderLesson();updateProgress();events()}catch(e){console.error(e);$("wordPreview").innerHTML="<p>Data belum dapat dimuat.</p>"}}
